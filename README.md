...
1. 注册并登录 EdgeOne Pages 平台。
2. 通过“导入 Git 仓库”方式连接本项目。
3. 平台会自动识别 edgeone.json 并按如下命令构建和启动：
   - 构建命令：npm install && npm run build
   - 启动命令：node app.js
4. 部署成功后，即可全球访问你的 NodeBB 社区。

如需 KV 存储、Pages Function、缓存等高级能力，请参考 EdgeOne 官方文档：https://edgeone.ai/zh/document/171937194382536704?product=edgedeveloperplatform

## EdgeOne KV 存储配置

- 在 EdgeOne 控制台获取 KV API Token、Endpoint、Namespace
- 部署时设置环境变量：
  - EDGEONE_KV_ENDPOINT
  - EDGEONE_KV_TOKEN
  - EDGEONE_KV_NAMESPACE

插件会自动将 Bug 卡片等数据存储到 EdgeOne KV，无需传统数据库。
